  ...
  return (
    ...
    >
      ...
      <Dialog open={!!activeProject} onOpenChange={(open) => !open && setActiveProject(null)}>
        <DialogContent
          className="sm:max-w-5xl w-[95vw] p-0 overflow-hidden bg-forest-dark border border-white/15 text-white"
          showCloseButton
        >
          {activeProject && (
                ...
                <Carousel opts={{ loop: true }} className="w-full">
                  <CarouselContent>
                    {activeProject.galleryImages.map((image, imageIndex) => (
                      <CarouselItem key={`${activeProject.id}-${image}-${imageIndex}`}>
                        <div className="overflow-hidden rounded-lg border border-white/10">
                          <img
                            src={image}
                            alt={`${activeProject.title} - gallery image ${imageIndex + 1}`}
                  ...
                  <CarouselPrevious className="left-3 top-1/2 -translate-y-1/2 border-white/40 bg-black/40 text-white hover:bg-white hover:text-forest-dark" />
                  <CarouselNext className="right-3 top-1/2 -translate-y-1/2 border-white/40 bg-black/40 text-white hover:bg-white hover:text-forest-dark" />
                </Carousel>
              </div>

              <div className="p-6 md:p-8 flex flex-col justify-center">
                <div className="flex items-center gap-3 mb-4 text-white/60 text-sm font-body">
                  <span>{activeProject.category}</span>
                  <span className="w-1 h-1 rounded-full bg-white/40" />
                  <span>{activeProject.year}</span>
                </div>
                <DialogTitle className="text-2xl md:text-3xl font-sans font-bold tracking-tight text-white mb-4">
                  {activeProject.title}
                </DialogTitle>
                ...
                <div className="space-y-3">
                  {activeProject.highlights.map((highlight) => (
                    <div key={highlight} className="flex items-start gap-3 text-white/80">
                      <span className="mt-2 h-1.5 w-1.5 rounded-full bg-white/60 shrink-0" />
                      <p className="text-sm md:text-base leading-relaxed">{highlight}</p>
                    </div>
                  ))}
                </div>
              </div>
